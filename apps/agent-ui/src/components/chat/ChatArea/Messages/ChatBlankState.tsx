  ...
  return (
    ...
    >
      <div className="flex max-w-3xl flex-col gap-y-8">
        ...
        >
          <div className="flex items-center justify-center gap-x-2 whitespace-nowrap font-medium">
            <span className="flex items-center font-[600]">
              OpenClaude Web runs fully local and keeps the native toolchain
          ...
          <p className="mt-4 text-base font-normal text-muted">
            Configure providers in Settings or reopen an existing session from
            the sidebar to continue working in the same OpenClaude runtime.
          </p>
        </motion.h1>
        <motion.div
          initial={{ opacity: 0, y: 10 }}
        ...
        >
          <ActionButton
            href={`${selectedEndpoint}${EXTERNAL_LINKS.documentation}`}
            variant="primary"
            text="LOCAL DOCS"
          />
          <ActionButton
            href={`${selectedEndpoint}${EXTERNAL_LINKS.status}`}
            text="CHECK STATUS"
          />
        </motion.div>
      </div>
